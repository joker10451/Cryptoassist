...
- Return valid JSON only, no markdown fences, no commentary`
    ...
    const projects = rawProjects
      ...
      .map((p: any) => ({
        ...
        slug: typeof p.slug === 'string' && p.slug.length > 0 ? slugify(p.slug) : slugify(p.name),
        ...
        website_url: typeof p.website_url === 'string' && p.website_url.startsWith('http') ? p.website_url : null,
        twitter_url: typeof p.twitter_url === 'string' && p.twitter_url.startsWith('http') ? p.twitter_url : null,
        tasks: Array.isArray(p.tasks)
          ? p.tasks
              .filter((t: any) => t && typeof t.title === 'string' && t.title.trim().length > 0)
              .map((t: any) => ({
                title: String(t.title).trim().slice(0, 200),
                task_type: VALID_TASK_TYPES.includes(t.task_type) ? t.task_type : 'custom',
    ...
    const totalTasks = projects.reduce((s: number, p: any) => s + p.tasks.length, 0)
